...
z = np.loadtxt('BPstep.nastruct.dat',unpack = True,usecols=(8),skiprows=(1))
df = pd.DataFrame.from_dict(np.array([x1,y,z]).T)
df.columns = ['Time (ns)','Basepairs','Twists']
df['Twists'] = pd.to_numeric(df['Twists'])
pivotted= df.pivot('Basepairs','Time (ns)','Twists')
sns.heatmap(pivotted,cmap='RdBu')
plt.title("Width of Twists in DNA (Angstroms)")
plt.show()
